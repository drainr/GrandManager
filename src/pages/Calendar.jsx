...
const Calendar = () => {
  ...
  const getTimeKey = (dayName, itemText, occurrenceIndex) =>
  ...
  useEffect(() => {
    ...
    const loadSavedEntries = async () => {
      try {
        const { entriesByDay, timesByKey, checkedByKey } =
          await getEntriesByDay(user.uid);
        if (isMounted) {
  ...
  return (
    <>
      <div className="flex flex-col lg:flex-row justify-center relative w-full overflow-hidden">
        <div className="mx-auto w-full max-w-5xl lg:max-w-10xl bg-[#1B2851] mt-25 px-3 lg:px-6 p-4 lg:p-6 rounded-xl shadow-2xl shadow-black lg:scale-90">
          <div className="relative flex items-center justify-center pb-10 pt-2">
            <div className="absolute left-0 top-1/2 -translate-y-1/2 scale-65 lg:scale-75 origin-left">
              <GreenButton text="← Back" onClick={() => navigate("/")} />
            </div>

            <h2 className="text-xl lg:text-2xl font-bold text-[#EBB537] shrikhand-regular">
              Calendar
            </h2>
          </div>
          ...
          <div className="mt-5">
            <DisplayDailyList
              ...
              onEditTime={(day, index, newTime) =>
                handlers.handleEditTime(
                  day,
                  index,
                  newTime,
                  dayMenus,
              ...
              onToggleItemChecked={(day, index, nextChecked) =>
                handlers.handleToggleTodoChecked(
                  day,
                  index,
                  nextChecked,
                  dayMenus,
          ...
          {/* Send menu — inside the card on mobile + tablet only */}
          <div className="mt-6 flex justify-center lg:hidden">
            <div className="bg-[#405BA4] p-4 rounded-xl shadow-xl flex flex-col gap-1 items-center w-full">
              <Send />
            </div>
          </div>
        </div>

        {/* Send menu — side panel on desktop only (original positioning) */}
        <div className="hidden lg:flex absolute top-1/2 bg-[#1B2851] p-5 rounded-xl -translate-y-1/2 flex-col gap-1 items-end scale-90 origin-right shadow-2xl right-5 shadow-black">
          <Send />
        </div>
      </div>
      <Footer />
