...
const Login = () => {
    ...
    const handleChangePassword = async (e) => {
        ...
        if (!email) {
            setChangeMsg("Enter your email address above first.");
            return;
        }
    ...
    return (
        <div className='bg-[#1B2851] w-100 shadow-xl shadow-black'>
            <div className="max-w-md w-full overflow-hidden p-8 space-y-8 ">
                ...
                {showChangePassword && (
                    <div className="change-password-section space-y-4">
                        <p className="text-gray-200 text-sm">Enter your email above and click below to receive a password reset link.</p>
                        <BlueButton text={'Send Reset Email'} onClick={handleChangePassword} />
                        {changeMsg && <p style={{ color: changeMsg.includes("sent") ? "green" : "red", fontSize: "12px", margin: "5px" }}>{changeMsg}</p>}
                    </div>
                )}
                {changeMsg && !showChangePassword && <p style={{ color: "red", fontSize: "12px", textAlign: "center" }}>{changeMsg}</p>}
